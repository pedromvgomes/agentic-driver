// Package claudecode is the Claude Code dialect: the flags `claude` takes, the
// shape of the envelope it prints, the environment variables that carry its
// credential and the ones that can redirect it somewhere else.
//
// It vendors its own copy of the CLI rather than using whatever is on PATH, at
// an exact pinned version verified against Anthropic's signed manifest. The pin
// is what makes the envelope schema and the flag spelling something this
// package can rely on, and an absolute path is what stops a PATH entry or a
// repointed symlink from substituting a different build.
package claudecode
...
// SigningKeyFingerprint is the only key a manifest is accepted from.
//
...
// other at startup is what stops swapping the file silently changing who is
// trusted.
//
...
//
// Informational only: the installer fetches its PINNED version, which may
// deliberately differ.
...
func (c *releaseClient) Manifest(ctx context.Context, version string) (Manifest, error) {
	...
	// keyring and returns which one. Discarding that answer rests the whole
	// guarantee on the keyring having exactly one member: appending a second
	// armored key to the embedded file would make it a fully trusted signer
	// while every other check still passed, defeating the
	// "swapping the file cannot change who is trusted" property the embedded
