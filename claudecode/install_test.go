...
// alone cannot silently change who is trusted.
...
func TestStableReportsTheBucketsCurrentVersion(t *testing.T) {
	...
	// The installer fetches the pin, not whatever the bucket calls current.
	if got != b.version {
...
// traversing version otherwise reaches the fast path, finds an existing
// `claude` outside the providers root, and reports an unverified binary as
// installed — precisely the hijack this package exists to prevent.
